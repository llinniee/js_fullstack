<template>
  <div class="play" v-show="playList.length>0">
    ...
    <audio src="" ref="audio" @playing="ready" @error="error" @timeupadte="updateTime"
    @ended="end" @pause="pause"></audio>
  </div>
</template>
...
import api from '@/api'
import { playerMixin} from '@/components/mixin'
  ...
  methods: {
    ...
    getLyric (id) {
      api.MusicLyric(id).then(res => {
        if (res.code === 200) {
          this.currentLyric = new Lyric(res.lrc.lyric, this.handLyric())
